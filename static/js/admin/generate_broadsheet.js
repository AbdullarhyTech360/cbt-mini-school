document.addEventListener('DOMContentLoaded', function () {
    ...
    // Load configs for broad sheet
    window.loadConfigsForBroadSheet = async function () {
        try {
            const response = await fetch('/reports/api/configs');
            const data = await response.json();

            if (data.success) {
                const configSelect = document.getElementById('broad-sheet-config');
                // Keep default option
                configSelect.innerHTML = '<option value="">Default (No Merging)</option>';
            ...
            // Don't show alert to avoid annoying user if configs fail silently
        }
    }
    ...
    async function loadAndPreviewBroadSheet() {
        ...
        if (!classId || !termId) {
            showAlert({ title: 'Error', message: 'Please select both class and term', type: 'error' });
            return;
        }
        ...
        if (examTypes.length === 0) {
            showAlert({ title: 'Error', message: 'Please select at least one exam type', type: 'error' });
            return;
        }

        showLoadingState('preview-broad-sheet', 'Analyzing Data...');

        try {
            const configId = document.getElementById('broad-sheet-config')?.value;
            ...
            hideLoadingState('preview-broad-sheet');
        }
    }
    ...
    async function exportBroadSheetFromData(format, broadSheetData) {
        ...
        try {
            ...
            const studentsPerPage = document.getElementById('students-per-page')?.value || 25;
            const fontSize = document.getElementById('broadsheet-font-size')?.value || 9;
            ...
            const response = await fetch(`/reports/api/broad-sheet/export/${format}`, {
                ...
                body: JSON.stringify({
                    ...
                    show_totals: showTotals,
                    config_id: configId,
                    subjects_per_page: parseInt(subjectsPerPage),
                    students_per_page: parseInt(studentsPerPage),
    ...
    async function showCanvasBasedPreviewBroadSheet(broadSheetData) {
        try {
            ...
            if (previewContainer) {
                ...
                // Render header separately so we can overlay it on every page
                let headerCanvas = null;
                let headerHeightPx = 0;
                const headerElem = tempDiv.querySelector('.header');
                if (headerElem) {
                    try {
                        headerCanvas = await html2canvas(headerElem, {
                            scale: 2,
                            useCORS: true,
                            allowTaint: true,
                            backgroundColor: "#ffffff",
                            logging: false,
                            windowWidth: headerElem.scrollWidth,
                            windowHeight: headerElem.scrollHeight,
                        });
                        headerHeightPx = headerCanvas.height;
                    } catch (e) {
                        console.warn('Failed to render header separately for page overlays', e);
                        headerCanvas = null;
                        headerHeightPx = 0;
                    }
                }
                ...
                for (let page = 0; page < totalPages; page++) {
                    ...
                    // Draw the page slice from the full canvas
                    ctx.drawImage(
                        canvas,
                        0,
                        page * a4HeightPx,
                    ...
                    // Overlay the header onto every page so class/term/session and page number appear
                    if (headerCanvas) {
                        try {
                            ctx.drawImage(
                                headerCanvas,
                            ...
                            console.warn('Failed to overlay header on page canvas', e);
                        }
                    }
            ...
            <div class="header">
                <div class="header-banner">
                    <div class="logo-placeholder">
                        ${metadata.school_logo
                ? `<img src="/${metadata.school_logo}" class="logo-img">`
                : `<span style="color:#4f46e5; font-weight:bold; font-size:30pt;">🏫</span>`
            }
                    </div>
                    <div class="school-text">
                        <h1 class="school-name">${metadata.school_name || "SCHOOL NAME"}</h1>
                        <div class="school-sections">
                            ${metadata.formatted_sections ? metadata.formatted_sections : ''}
                        </div>
                        <div class="school-motto">Motto: ${metadata.school_motto || "N/A"}</div>
                    </div>
                </div>
                <div style="font-size: 18px; font-weight: 700; margin: 15px 0; color: #1e293b; letter-spacing: 4px;">OFFICIAL BROAD SHEET</div>
    ...
    const updateExamTypesState = () => {
        const allEle = document.getElementById('exam-type-all');
        const caEle = document.getElementById('exam-type-ca');
        const examEle = document.getElementById('exam-type-exam');

        if (!allEle || !caEle || !examEle) return;

        if (allEle.checked) {
            caEle.checked = false;
            caEle.disabled = true;
            examEle.checked = false;
            examEle.disabled = true;
        } else {
            caEle.disabled = false;
            examEle.disabled = false;
        }
    };

    document.getElementById('exam-type-all')?.addEventListener('change', updateExamTypesState);

    // Initial state check
    updateExamTypesState();
